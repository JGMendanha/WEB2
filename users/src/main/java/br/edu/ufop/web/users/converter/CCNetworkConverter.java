...
public class CCNetworkConverter {
    ...
    // DTO de entrada para a classe do dominio
    public static CCNetworkDomain toDomain(CreateCreditCardNetworkDTO createCreditCardNetworkDTO) {

        return CCNetworkDomain.builder().name(createCreditCardNetworkDTO.name()).build();
    }

    // Classe do dominio para entidade JPA
    public static CCNetworkEntity toEntity(CCNetworkDomain ccNetworkDomain) {

        return CCNetworkEntity.builder().id(ccNetworkDomain.getId()).name(ccNetworkDomain.getName()).build();
    }
